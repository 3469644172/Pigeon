...
class Pigeon {
	...
	public function getTimeline($username, $displayHtml = true, $page = 1) {
		...
		if($displayHtml) {
			...
			while($rw = mysqli_fetch_row($rs)) {
				...
				$html .= "<tr><td class='headimg'><img src='https://q1.qlogo.cn/g?b=qq&nk=" .$this->getUserInfo($rw[2])['qq'] . "&s=640'</td><td class='thread'><p><small>".$this->getUserInfo($rw[2])['name']." 发表于" . $this->getDateFormat($rw[3]) . "&nbsp;&nbsp;<a href='?s=msg&id={$rw[0]}' target='_blank'><i class='fa fa-external-link'></i></a>" . $pstatus . str_replace("{id}", $rw[0], $sdelete) . "</small></p>";
	...
	public function getMessageById($id) {
		...
		if($rs) {
			...
			$html .= "<tr><td class='headimg'><img src='http://q1.qlogo.cn/g?b=qq&nk=" . $this->getUserInfo($rs['author'])['qq'] . "&s=640'</td><td class='thread'><p><small>".$this->getUserInfo($rs['author'])['name'] ."发表于" . $this->getDateFormat($rs['time']) . $pstatus . str_replace("{id}", $id, $delete) . "</small></p>";
