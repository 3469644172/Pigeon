...
if(!isAdmin($_SESSION['user'])) {
	echo "<html><head><title>权限不足...</title></head><body><script>window.alert('权限不足！');window.location='5,/';</script></body></html>";
	exit;
}
if(isset($_GET['s'])) {
	switch($_GET['s']) {
		...
		case "userlist":
			...
			<tbody>
				...
				while($rw = mysqli_fetch_row($rs)) {
					...
					<td nowrap>{$rw[5]}</td>
					<td nowrap>{$rw[6]}</td>
					<td nowrap>" . AccountStatus($rw[11]) . "</td>
			...
			if(isset($_GET['id']) && preg_match("/^[0-9]{0,10}$/", $_GET['id'])) {
				...
				if($rs) {
					if(isset($_POST['username']) && isset($_POST['email']) && isset($_POST['permission']) && isset($_POST['status'])) {
						...
						$name = mysqli_real_escape_string($pigeon->conn, $_POST['name']);
						$qq = mysqli_real_escape_string($pigeon->conn, $_POST['qq']);
						...
						mysqli_query($pigeon->conn, "UPDATE `users` SET `username`='{$username}',`qq`='{$qq}',`name`='{$name}',`email`='{$email}',`permission`='{$permission}',`status`='{$status}'{$passwordSQL} WHERE `id`='{$uid}'");
						echo "Successful";
					} else {
						ErrorMsg("请将用户信息填写完整");
					}
		...
		case "updatecheck":
			...
			$update = @file_get_contents("https://api.zeroteam.top/pigeon/");
			echo $update;
			break;
		case "updateexecute":
			if(!isset($_GET['seid']) || $_GET['seid'] !== $_SESSION['seid']) {
		...
		<div class="container">
			<div class="row">
				...
				<div class="col-sm-3">
					...
					<p>QQ</p>
					<p><input type="text" id="qq" class="form-control"></p>
					<p>密码</p>
				...
				<div class="col-sm-12">
					<hr>
					<p>&copy; <?php echo date("Y"); ?> <?php echo $pigeonConfig['sitename']; ?></p>
				</div>
			</div>
			<style type="text/css">
				.margin-footer {
